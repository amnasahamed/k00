...
export const generateAlerts = (
  ...
  assignments.forEach((assignment) => {
    ...
    // Warning: Deadline approaching (24-48h) — applies to any active task,
    // not just In Progress. A Pending task with a tight deadline is exactly
    // when an admin most needs a heads-up.
    if (
      hoursUntilDeadline > 24 &&
    ...
    // Warning: Payment due soon — any active assignment with a near-term
    // deadline and an outstanding balance should raise a flag, not only
    // tasks already moved to In Progress.
    if (
      remainingAmount > 0 &&
      hoursUntilDeadline > 0 &&
...
export const generateActionItems = (
  ...
  // Tasks due today — include any non-completed, non-cancelled task whose
  // deadline falls in today's window, not just In Progress tasks.
