                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <TabsContent value="details" className="space-y-6">
                        <div className="space-y-4">
                            ...
                            <Input label="Deadline" type="date" required value={assignment.deadline ? new Date(assignment.deadline).toISOString().split('T')[0] : ''} onChange={e => {
                                // The user can clear the field while typing;
                                // `new Date('').toISOString()` throws a
                                // RangeError, so guard for empty input.
                                if (!e.target.value) {
                                    onAssignmentChange({ ...assignment, deadline: '' });
                                    return;
                                }
                                onAssignmentChange({ ...assignment, deadline: new Date(e.target.value).toISOString() });
                            }} />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-success/5 p-4 rounded-apple border border-success/10 space-y-3">
                                    <Input label="Word Count" type="number" min={0} value={assignment.wordCount} onChange={e => onAssignmentChange({ ...assignment, wordCount: Math.max(0, Number(e.target.value) || 0) })} className="bg-white border-success-100/30" />
                                    <div className="pt-2 border-t border-success/10">
                                        <Input label="Total Price" type="number" min={0} value={assignment.price} onChange={e => onAssignmentChange({ ...assignment, price: Math.max(0, Number(e.target.value) || 0) })} className="bg-white border-success-100/50 font-bold" />
                                        <Input label="Paid Amount" type="number" min={0} value={assignment.paidAmount} onChange={e => onAssignmentChange({ ...assignment, paidAmount: Math.max(0, Number(e.target.value) || 0) })} className="mt-2 bg-white border-success-100/50" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {assignment.writerId && (
                                    <div className="bg-danger/5 p-4 rounded-apple border border-danger/10 space-y-3 animate-in fade-in slide-in-from-top-2">
                                        <Input label="Writer Rate/Word" type="number" min={0} step="0.1" value={assignment.writerCostPerWord} onChange={e => onAssignmentChange({ ...assignment, writerCostPerWord: Math.max(0, Number(e.target.value) || 0) })} className="bg-white border-danger-100/30" />
                                        <div className="pt-2 border-t border-danger/10">
                                            <Input label="Writer Fee" type="number" min={0} value={assignment.writerPrice} onChange={e => onAssignmentChange({ ...assignment, writerPrice: Math.max(0, Number(e.target.value) || 0) })} className="bg-white border-danger-100/30 font-bold" />
                                            <Input label="Paid to Writer" type="number" min={0} value={assignment.writerPaidAmount} onChange={e => onAssignmentChange({ ...assignment, writerPaidAmount: Math.max(0, Number(e.target.value) || 0) })} className="mt-2 bg-white border-danger-100/30" />
                                        </div>
