...
import { WriterPerformanceProfile, DashboardPerformance, Assignment, Achievement, AssignmentStatus } from '../types';
...
const WriterDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const writerId = user?.id || (() => {
      if (!storedWriterData) return null;
      try {
        const parsed = JSON.parse(storedWriterData);
        return (parsed.writer || parsed)?.id;
      } catch {
        return null;
      }
    })();
    ...
    if (storedWriterData) {
      try {
        const parsed = JSON.parse(storedWriterData);
        setWriter(parsed.writer || parsed);
      } catch {
        // ignore
      }
    }

    fetchDashboardData(writerId);
  }, [navigate, user?.id]);

  const fetchDashboardData = async (writerId: number | string) => {
    try {
  ...
  const filteredAssignments = assignments.filter((assignment) => {
    ...
    // Compare against the canonical AssignmentStatus enum values rather
    // than loose lowercase strings — the previous `toLowerCase()` chain
    // would silently match "in-progress" and miss "In Progress" (with a
    // space) depending on how the status was stored.
    if (activeTab === 'Completed') return assignment.status === AssignmentStatus.COMPLETED;
    if (activeTab === 'In Progress') return assignment.status === AssignmentStatus.IN_PROGRESS;
    if (activeTab === 'Pending') return assignment.status === AssignmentStatus.PENDING;
    return true;
  });
