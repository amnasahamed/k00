...
const Layout: React.FC = () => {
  ...
  const primaryNav = [
    { to: '/dashboard', label: 'Dashboard', short: 'Home', icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
    { to: '/students', label: 'Students', short: 'Students', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
    { to: '/assignments', label: 'Tasks', short: 'Tasks', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
    { to: '/payments', label: 'Payments', short: 'Pay', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    { to: '/writers', label: 'Writers', short: 'Writers', icon: 'M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z' },
  ];
  ...
  const allNav = [...primaryNav, ...moreNav.map((item) => ({ ...item, short: item.label, icon: '' }))];

  const getPageTitle = () => {
    const current = allNav.find((item) => item.to === location.pathname);
    ...
    <SearchContext.Provider value={{ openSearch: () => setSearchOpen((prev) => !prev) }}>
      <div className="min-h-screen bg-background text-secondary-900 font-sans selection:bg-primary-500/20">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-secondary-200 z-50 pb-safe shadow-[0_-1px_10px_rgba(0,0,0,0.05)]">
          <div className="grid grid-cols-5 h-16">
            ...
            <button
              ...
              className={`flex flex-col items-center justify-center gap-1 transition-colors ${
                moreOpen || moreNav.some((n) => n.to === location.pathname)
                  ? 'text-primary-600'
          ...
          {moreOpen && (
            <div className="absolute bottom-16 inset-x-3 rounded-apple-lg border border-secondary-200 bg-white shadow-ios p-2">
              {[primaryNav[4], ...moreNav].map((item) => (
                <NavLink
                  key={item.to}
                  to={item.to}
                  onClick={() => setMoreOpen(false)}
                  className={({ isActive }) =>
                    `block px-3 py-2.5 rounded-apple text-sm font-medium ${
