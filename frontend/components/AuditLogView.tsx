...
import { getAssignments, getStudents } from '../services/dataService';
import { Assignment, Student, ActivityLogEntry, PaymentHistoryEntry, StatusHistoryEntry } from '../types';
...
const AuditLogView: React.FC = () => {
  ...
  const fetchAuditLogs = async () => {
    try {
      setLoading(true);
      const [assignments, students] = await Promise.all([getAssignments(), getStudents()]);

      // Map studentId -> name once so each log row can resolve the name
      // without an O(n) find over the students list. Previously the view
      // displayed the raw studentId, making every row read like a debug
      // dump instead of a human-readable audit entry.
      const studentNameById = new Map<string, string>(
        students.filter(s => s.id).map(s => [s.id, s.name])
      );

      const auditLogs: AuditLogItem[] = [];

      assignments.forEach(assignment => {
        const studentName = studentNameById.get(assignment.studentId) || 'Unknown Student';
