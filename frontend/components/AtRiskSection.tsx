          ...
          >
            ...
            {alert.actionLabel && (
              <button
                type="button"
                onClick={(e) => {
                  // The whole card is clickable and already navigates via
                  // handleAlertClick; this button shares the same action so
                  // users hitting the explicit CTA get the same behavior.
                  e.stopPropagation();
                  handleAlertClick(alert);
                }}
                className="flex-shrink-0 px-3 py-1.5 bg-white/50 hover:bg-white/80 rounded-[8px] text-xs font-medium transition-all duration-200 ease-apple active:scale-95"
              >
                {alert.actionLabel}
              </button>
            )}
