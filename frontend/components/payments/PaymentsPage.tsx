...
import { useAssignments, useSaveAssignment } from '../../hooks/useAssignments';
...
const PaymentsPage: React.FC = () => {
    ...
    // Re-derive finance charts whenever data or the date-range filter changes.
    useEffect(() => {
        if (assignments.length || students.length || writers.length) {
            calculateFinanceData(assignments, students, writers);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [dateRangeFilter, assignments, students, writers]);
    ...
    const calculateFinanceData = React.useCallback((data: Assignment[], studentList: Student[], writerList: Writer[]) => {
        ...
        const matrix = writerList.map(w => {
            // Coerce to string: backend Writer.id is a number, but
            // Assignment.writerId can come back as either number or string.
            // A strict `===` would silently drop the writer's own
            // assignments, breaking the cost-vs-quality scatter plot.
            const wAssignments = data.filter(a => String(a.writerId) === String(w.id));
    ...
    const handlePayment = async (e: React.FormEvent) => {
        ...
        // NaN check: `Number('')` is 0 but `Number('abc')` is NaN. NaN
        // never satisfies `< 0` or `> price`, so without this guard a
        // user could submit a non-numeric amount and silently corrupt
        // the paidAmount / writerPaidAmount field.
        if (Number.isNaN(amount)) {
            addToast('Payment amount must be a number', 'error');
            return;
        }

        // Validate that a writer is assigned for outgoing (writer) payments.
        // The previous check required the *student* name, which is unrelated
        // to paying the writer and blocked legitimate payouts when the
        // student record was missing.
        if (activeTab === 'outgoing' && !selectedAssignment.writerId) {
            addToast('No writer assigned to this assignment', 'error');
            return;
        }
        ...
        if (isEditMode) {
            ...
            // Append mode: a single transaction can't pay more than the
            // remaining balance, otherwise the books drift past zero.
            if (activeTab === 'incoming') {
                const remaining = selectedAssignment.price - selectedAssignment.paidAmount;
                if (amount > remaining) {
                    addToast(`Payment exceeds remaining balance of ${formatCurrency(remaining)}`, 'error');
                    return;
                }
            } else {
                const remaining = (selectedAssignment.writerPrice || 0) - (selectedAssignment.writerPaidAmount || 0);
                if (amount > remaining) {
                    addToast(`Payment exceeds remaining writer balance of ${formatCurrency(remaining)}`, 'error');
                    return;
                }
            }
        }
        ...
        try {
            await saveAssignmentMutation.mutateAsync(updatedAssignment);
