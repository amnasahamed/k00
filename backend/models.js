...
const Writer = sequelize.define('Writer', {
  ...
  hooks: {
    beforeValidate: async (writer, options) => {
      // Check if phone is missing or invalid (not 10 digits)
      const isValidPhone = writer.phone && /^\d{10}$/.test(writer.phone);

      if (!isValidPhone) {
        // Use a unique random placeholder per writer to avoid race conditions
        // on bulk inserts. Format: 0000 + 6 random digits (still satisfies the
        // not-10-digit placeholder shape so it can be replaced later by a
        // real phone if one is supplied).
        const random = Math.floor(Math.random() * 1_000_000).toString().padStart(6, '0');
        writer.phone = `0000${random}`;
      }
    }
  }
});
